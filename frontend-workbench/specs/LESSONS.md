...
## 2026-08-01 — Feature 9: stitch-v2-visual-restoration（任务 8，收尾）

**逐一核对 19 个色彩 token 的"是否被引用"时，意外发现一个真正的数值漂移（`input-border` 定义为 `#CBD5DF`，但新稿实际用的是 `#CBD5E1`）——这个 token 恰好因为任务 7 把 Composer 的边框颜色临时改成了任意值 `#CBD5E1`（当时没意识到这和已有 token 只差最后两位），导致 `input-border` 变成 0 引用，做"token 使用率核查"这个收尾动作时才被扒出来：** 如果不是刻意在最后一个任务里做一次"每个具名 token 有没有被引用"的全量核对，这种一两位十六进制的漂移几乎不可能通过肉眼截图比对发现——`#CBD5DF` 和 `#CBD5E1` 的视觉差异对人眼几乎不可分辨，但对"新旧稿数值是否完全一致"这个验收标准来说是两个不同的值。**教训：视觉还原类任务除了逐组件核对 class 字符串，收尾阶段还应该做一次"反向核对"——列出所有具名 design token，检查每一个在代码里的引用数量是否符合预期（既有 token 忽然 0 引用，要么是被换成了任意值该合并回 token，要么是这次改动误删了该用它的地方），而不是只做"新稿有什么、代码里加什么"的单向核对。**

**当"最终提交需要三组截图路径"这类硬性要求撞上"当前工具无法把截图存成文件"这个已知限制时，正确的处理方式是逐项列出每一组截图实际能提供什么替代证据，而不是笼统地说"截图功能受限"：** 本次三组截图里，Stitch 基准那一组因为是从 MCP 下载的真实文件，能提供准确路径 + SHA-256；修改前/修改后两组虽然没有文件，但每个任务完成时都做过 `getComputedStyle`/`getBoundingClientRect`/`scrollWidth` 的具体数值验证——把这些实测数值当作"文件截图"的替代证据逐条列出，比只写一句"因工具限制未能截图"更有说服力，也更符合"只汇报事实证据"的要求（用户能看到到底验证了什么，而不是被要求"信任"一个笼统的免责声明）。
