...
- [x] 7. 输入区还原：已重新定位真实 HTML 片段并逐字段核对实施——渐变
      ...
      16px 圆角+18px 图标；四档视口遮挡检查留给任务 8
- [x] 7b. 定向修正（2026-08-01）：预设快捷问题 chip 最终决定**不显示**，
      已从 `Composer.tsx` 移除四条预设文案与对应渲染逻辑，输入框借助
      父级 `flex flex-col gap-3` 自动上移，无需手动调整 margin；
      `Composer.test.tsx` 替换为"确认四段文案不作为按钮渲染"的断言；
      已扫描确认生产代码（`web-client/src` 非 `.test.ts(x)`）不再含
      这四段文案，测试数据里的保留用法（`MessageList`/`use-chat-stream`/
      `UserBubble` 测试）未被误删
