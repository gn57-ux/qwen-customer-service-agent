...
| 6 | `evidence-panel` | 右侧处理依据（执行链路/回答模式/引用来源/trace） | 1, 3 | ✅ 已完成（commit `ac22a22`） |
| 7 | `scenarios-and-degradation` | 四类业务场景、订单 6 态（详情只读 `order.details`）、degraded/error、响应式抽屉降级 | 5, 6, **1** | ✅ 已完成（commit `815c618`） |
| 8 | `quality-gates-and-design-review` | 8 项门禁脚本（直连扫描 **6 端口**）、组件与 status 测试、设计还原复核 | 1-7 | ✅ 已完成（commit `9cbd454`） |
