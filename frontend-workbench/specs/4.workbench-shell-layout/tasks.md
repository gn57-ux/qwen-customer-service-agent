...
- [x] T-001: 新建 `hooks/use-service-status.ts`：初始三项 `unknown`、挂载即探测、catch 落为三项 `error` 且不抛出 ~30min
- [x] T-002: 实现刷新时机 —— 暴露 `refresh()` 供对话 `done`/`error` 调用；可选轮询 ≥30s 且 `visibilityState==="hidden"` 时暂停 ~15min

### 功能 2: 顶栏

- [x] T-003: `<Header />` 三组骨架 + 左组标题与 JetBrains Mono 副标题（`hidden md:inline`） ~30min
- [x] T-004: 三项服务状态渲染：四态标签映射 + 8×8px 0 圆角状态点 + 颜色映射，文字与颜色同时传达状态 ~30min
- [x] T-005: 「清空会话」按钮（`delete` 18px + `hidden md:inline` 文案 + hover/active 态） ~15min

### 功能 3: 左栏与主网格

- [x] T-006: `<LeftSidebar />`：220px 固定宽、`justify-between` 两端布局、新建会话按钮、会话列表（激活态 2px 竖条 + bold + truncate）、底部本地运行文案 ~30min
- [x] T-007: 主网格容器（`flex-1 mt-16 max-w-[1920px] mx-auto overflow-hidden`）接入三栏，验证 body 无滚动 ~15min
